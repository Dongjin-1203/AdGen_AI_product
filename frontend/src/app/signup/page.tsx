...
export default function Signup() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // 1. 회원가입 요청
      const signupResponse = await fetch(`${API_URL}/api/v1/signup`, {
        method: 'POST',
        headers: {
        ...
        body: JSON.stringify({
          ...
          password,
          name,
          phone: phone || undefined,  // 빈 문자열이면 undefined
        }),
      });
      ...
      if (!loginResponse.ok) {
        // 로그인 실패 시 로그인 페이지로
        alert('회원가입은 성공했습니다! 로그인 페이지로 이동합니다.');
        router.push('/login');
        return;
      }
